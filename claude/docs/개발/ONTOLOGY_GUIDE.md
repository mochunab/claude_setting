...
> **목적**: 도메인 지식을 구조화된 그래프(온톨로지)로 구축하고 시각화하기 위한 단계별 가이드
> **작성일**: 2026-03-17
...
| 도메인 | 범위 | 활용 |
|--------|------|------|
| **서비스 아키텍처** | 컴포넌트, API, DB, 페이지 간 관계 | 개발 생산성, 영향도 분석 |
| **도메인 지식** | 비즈니스 도메인의 핵심 개념과 관계 | AI 해석 품질 향상, 자동화 |
| **사용자 행동 흐름** | 인증 → 결제 → 콘텐츠 소비 → 전환 | 퍼널 분석, 개인화 추천 |
| **콘텐츠 관계** | 카테고리, 태그, 추천, 연관 관계 | 추천 시스템 고도화 |
...
APIEndpoint:
  ...
  - auth_required: boolean  # 인증 필요 여부
  - rate_limit: integer     # 분당 요청 제한
  - category: enum          # ai | payment | notification | report
...
| **호출 (calls)** | A → B | A가 B를 호출 | Component → APIEndpoint |
| **의존 (depends_on)** | A → B | A가 B에 의존 | Component → Library |
| **읽기/쓰기 (reads/writes)** | A → B | A가 B를 읽거나 씀 | APIEndpoint → DBTable |
| **연관 (related_to)** | A ↔ B | 양방향 연관 | Product ↔ Category |
| **생성 (generates)** | A → B | A가 B를 생성 | APIEndpoint → Report |
| **소속 (belongs_to)** | A → B | A가 B에 소속 | User → Organization |

#### 예시: 서비스 아키텍처 관계

```
ProductDetailPage   --contains-->  PaymentForm
PaymentForm         --calls-->     /api/payments (APIEndpoint)
/api/payments       --writes-->    orders (DBTable)
/api/ai-generate    --reads-->     user_profiles (DBTable)
/api/ai-generate    --writes-->    generated_results (DBTable)
/api/ai-generate    --uses_model-> GPT-5.1
```
...
meta_edges:
  ...
  # Library → APIEndpoint: ❌ (프론트 라이브러리가 서버 함수를 직접 호출 불가)
```
...
APIEndpoint: "/api/ai-generate"
  ...
  Relational:
    called_by: [ProductDetailPage, DashboardPage]
    writes_to: [generated_results]
    reads_from: [user_profiles, orders, products]
    depends_on: [OpenAI GPT-5.1, Stripe API]
  Behavioral:
    daily_calls: 150
...
```
MyCompany / my-company / mycompany  →  MyCompany (정규화)
ProductDetailPage / 상품 상세 페이지  →  ProductDetailPage
```
...
```cypher
// 페이지 노드
CREATE (:Page {name: 'HomePage', route: '/', file_path: '/pages/HomePage.tsx', auth_required: false, category: 'content', source_type: 'code', confidence: 1.0})
CREATE (:Page {name: 'ProductDetailPage', route: '/products/:id', file_path: '/pages/ProductDetailPage.tsx', auth_required: false, category: 'content'})

// 컴포넌트 노드
CREATE (:Component {name: 'PaymentForm', file_path: '/components/PaymentForm.tsx'})
CREATE (:Component {name: 'SearchBar', file_path: '/components/SearchBar.tsx'})

// API 엔드포인트 노드
CREATE (:APIEndpoint {name: '/api/ai-generate', method: 'POST', auth_required: true, category: 'ai'})
CREATE (:APIEndpoint {name: '/api/payments', method: 'POST', auth_required: true, category: 'payment'})

// DB 테이블 노드
CREATE (:DBTable {name: 'orders', description: '결제 주문'})
CREATE (:DBTable {name: 'generated_results', description: 'AI 생성 결과'})

// 카테고리 노드
CREATE (:Category {name: 'Electronics', slug: 'electronics', level: 1})
CREATE (:Category {name: 'Books', slug: 'books', level: 1})
```
...
```cypher
// 페이지 → 컴포넌트 포함
MATCH (p:Page {name: 'ProductDetailPage'}), (c:Component {name: 'PaymentForm'})
CREATE (p)-[:CONTAINS]->(c)

// 컴포넌트 → API 호출
MATCH (c:Component {name: 'PaymentForm'}), (api:APIEndpoint {name: '/api/payments'})
CREATE (c)-[:CALLS]->(api)

// API → DB 읽기/쓰기
MATCH (api:APIEndpoint {name: '/api/ai-generate'}), (t:DBTable {name: 'generated_results'})
CREATE (api)-[:WRITES]->(t)

// 상품 → 카테고리 소속
MATCH (p:Product {name: 'MacBook Pro'}), (c:Category {name: 'Electronics'})
...
```cypher
// 특정 API 엔드포인트의 전체 의존 관계 탐색 (3홉까지)
MATCH path = (start)-[*1..3]-(end)
WHERE start.name = '/api/ai-generate'
RETURN path
...
MATCH path = (p:Page)-[:CONTAINS]->(:Component)-[:CALLS]->(:APIEndpoint)-[:WRITES]->(t:DBTable)
RETURN path
...
[ProductDetailPage]
  ...
  │              ├──calls→ [/api/payments] ──writes→ [orders]
  │              └──contains→ [CouponSelector]
  │                            └──calls→ [/api/apply-coupon]
  ├──contains→ [ProductInfo]
  ├──navigates→ [CheckoutPage]
  │              └──polls→ [orders.payment_completed]
  └──navigates→ [OrderConfirmPage]
                  └──reads→ [order_results]
```

### 도메인 B: 사용자-콘텐츠 추천 그래프

```
    User ──views──→ Product ──belongs_to──→ Category
      │                │
      ├──purchases──→ Order ──contains──→ OrderItem
      │                                      │
      └──reviews──→ Review ──references──→ Product
```
...
├── Sub-agent: 콘텐츠 생성 (generate-content)
├── Sub-agent: 데이터 분석 (analyze-data)
├── Sub-agent: 요약 (summarize-report)
└── Skills
...
관계 (15~20개):
  ...
  - APIEndpoint READS/WRITES DBTable
  - Component CALLS APIEndpoint

필수 속성:
  - 모든 노드: id, name, status, source_type, confidence
  - Content: title, category, price
  - APIEndpoint: method, auth_required, category
```
