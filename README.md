...
│   │   │   └── SECURITY.md                    # 보안 가이드
│   │   ├── 바이럴_프레임워크.txt               # 에이전트 빠른 참조 (삼각형, 본능, CTA)
│   │   ├── 마케팅_용어.txt
│   │   ├── 서비스_기획.txt
│   │   └── 행동경제학.txt
│   └── settings.json
...
| Hook | 트리거 | 역할 |
|------|--------|------|
| **continuous-learning-v2** | `PostToolUse *` | 모든 도구 사용을 관찰 → 패턴 추출 → 본능(instinct) 생성 |
| **strategic-compact** | `PreToolUse Edit\|Write` | 도구 호출 50회 도달 시 `/compact` 제안 |
...
| `개발/HARNESS_ENGINEERING.md` | Claude Code 하네스 엔지니어링 |
...
| 파일 | 내용 |
|------|------|
| `바이럴_프레임워크.txt` | 바이럴 삼각형, 7대 심리 트리거, 7대 본능, CTA 규칙, 티어 판정 |
...
### settings.json
글로벌 설정 — permissions (allow/deny), hooks, 플러그인, 기본 모드(`acceptEdits`)
